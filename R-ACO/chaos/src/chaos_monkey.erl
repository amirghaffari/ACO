...
init([]) ->
	util:show_message(lists:flatten(io_lib:format("Running chaos monkey on node ~p with PID ~p ",[node(), self()]))),
...
handle_info(kill_something, State = #state{avg_wait = AvgWait, apps = Apps}) ->
    case do_kill(Apps) of
        {ok, KilledInfo} ->
            ...
            %%amir
			util:show_message(lists:flatten(io_lib:format("Killed process ~p",[KilledInfo])));
        {error, no_killable_processes} ->
            p("Warning: no killable processes.", []),
            %%amir
			util:show_message("Warning: no killable processes.")
    end,
...
handle_info(my_kill_something, State = #state{avg_wait = AvgWait, apps = _Apps, pids= PIDs}) ->
	case length(PIDs) of
		0 ->
			util:show_message(lists:flatten(io_lib:format("There is no PID to kill on node ~p",[node()]))),
			{noreply, State};
		_ ->
			%util:show_message(lists:flatten(io_lib:format("There is ~p PIDs to kill on node ~p",[length(PIDs), node()]))),
			SortedPIDs=randomize(PIDs),
			[PID|_Tail]=SortedPIDs,
			Reason=kill(PID),
			%%amir
			NewPIDs=lists:delete(PID, PIDs),
			util:show_message(lists:flatten(io_lib:format("Chaos Monkey kills a process ~p with reason ~p on node ~p - remain length is ~p",[PID, Reason, node(), length(PIDs)]))),
			Var = 0.3, %% I.e. 70% to 130% of Waittime
			WaitTime = round(AvgWait * ((1 - Var) + (Var * 2 * random:uniform()))),
...
handle_info({pid, NewPID}, State = #state{pids= PIDs}) ->
	NewPIDs=PIDs++[NewPID],
	%NewPIDs=PIDs,
	%util:show_message(lists:flatten(io_lib:format("gets pid ~p on node ~p added to list ~p new list ~p",[NewPID, node(), length(PIDs), length(NewPIDs)]))),
...
is_killable(Pid, App, AppFilter, IsSupervisorKillable)
  when is_pid(Pid), is_atom(App), is_boolean(IsSupervisorKillable) ->
      %%amir
    util:show_message(lists:flatten(io_lib:format("is killable process ~p ~n",[Pid]))),
    (App =:= undefined
     orelse
     case AppFilter of
         all -> true;
