...
%	    io:format("Solution ~p from ~p~n", [Cost, Pid]),
	...
	{'EXIT', FailedPID, Reason} -> 
		...
		{_MaxLevels=0,_NodeLength=0} -> 
			io:format ("Failure of a colony process with PID ~p detected by master process and reason ~p ~n", [FailedPID, Reason]);
		{_MaxLevels,_NodeLength} -> 
			io:format ("Failure of a sub-master process on the first level with PID ~p and reason ~p ~n", [FailedPID, Reason])
		end,
	...
	after
		?TimeOut ->
			io:format ("Timeout has occured in master process ~p on node ~p with Num_left ~p and received ~p ~n", [self(), node(), Num_left, ReceivedPIDs]),
			collect_colonies (Num_left, Current_Best, Master_state, Colonies, ReceivedPIDs, Duplicating)
    end.
...
recover_childs(FailedPID, Master_state, Colonies) ->
	...
	{ProcessName, ProcessIndex} ->
		...
		{_MaxLevels=0,_NodeLength=0} -> 
			...
			io:format ("recovery of a colony process ~p detected by master process on node ~p by new length ~p and new PID ~p ~n", [FailedPID, node(), length(NewChildProcesses), NewPID]);
		{MaxLevels,NodeLength} -> 
			{UsingNodes,_Rest}=lists:split(NodeLength, Nodes),
			[Head|_Tail]=UsingNodes,
			...
			util:send_pid(NewPID),
			NewChildProcesses=update_process_PID(ProcessName, NewPID, Colonies),
			io:format ("recovery of a sub-master process ~p detected by master process on node ~p by new length ~p and new PID ~p ~n", [FailedPID, node(), length(NewChildProcesses), NewPID])
		end,
