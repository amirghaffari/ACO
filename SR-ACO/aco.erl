...
start_aco (Fname, Vertex_degree, Duplicating, Num_Ants, Iter_Global, Iter_Local, NodeFile, Params0, Recovery) ->
    ...
    case util:dups(Nodes) of
		[] -> io:format ("No node duplication~n"), ok;
		D -> util:quit ("Error. Duplicate nodes in ~p: ~p.~n", [NodeFile,D])
    end,
    % This will print a message but not actually terminate the VM.
	[Head|_Tail]=Nodes,
	MasterPID=spawn_link(Head, ant_master, run, [Num_Jobs, Vertex_degree, Duplicating, Num_Ants, Iter_Global, Iter_Local, Inputs, Params, Nodes, Recovery, self()]),
	loop(MasterPID, Num_Jobs,Fname, Vertex_degree, Duplicating, Num_Ants,  Iter_Global,Iter_Local, NodeFile, Params0).
...
loop(MasterPID, Num_Jobs,Fname, Vertex_degree, Duplicating, Num_Ants,  Iter_Global,Iter_Local, NodeFile, Params0)->
	receive
		...
		{'EXIT', MasterPID, Reason} -> io:format ("Failure of the master process with PID ~p and reason ~p ~n", [MasterPID, Reason]),
			start_aco (Fname, Vertex_degree, Duplicating, Num_Ants, Iter_Global, Iter_Local, NodeFile, Params0, _Recovery=true)
	end.
...
run2(Args,Params) ->
	...
	[Fname,Vertex_degree0, Duplicating0, Num_Ants0, Iter_Global0, Iter_Local0, NodeFile] ->
		...
		timer:tc (fun() -> start_aco (Fname, Vertex_degree, Duplicating, Num_Ants, Iter_Global, Iter_Local, NodeFile, Params#params{parent_printer=none}, _Recovery=false) end),
