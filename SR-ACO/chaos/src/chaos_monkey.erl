...
on() ->
	%util:show_message(lists:flatten(io_lib:format("~p Waiting for receiving pids to kill .... ~n", [3]))),
    %gen_server:cast(?SERVER, {my_on, ?DEFAULT_OPTS}, infinity).
    gen_server:cast(?SERVER, {my_on, ?DEFAULT_OPTS}).
...
init([]) ->
	util:show_message(io_lib:format("Running chaos monkey on node ~p with Pid ~p ",[node(), self()])),
...
read_pids(PIDs) ->
	receive
		{pids,NewPIDs} -> 
			read_pids(PIDs++NewPIDs);
		{run_the_chaos, Starter} ->
			[Starter]++PIDs
	end.
...
handle_call({my_on, Opts}, _From, State = #state{is_active = false}) ->
    case verify_opts(Opts) of
        {ok, Ms, Apps} ->
			[Starter|PIDs]=read_pids([]),
			util:show_message(lists:flatten(io_lib:format("Getting ~p processes to kill on node ~p ",[length(PIDs), node()]))),
			NewState = State#state{avg_wait = Ms,
								   apps = Apps,
								   is_active = true,
								   pids=PIDs,
								   starter=Starter},
			self() ! my_kill_something,
			{reply, {ok, started}, NewState};
        {error, Error} ->
            {reply, {error, Error}, State}
    end;
...
handle_cast({my_on, Opts},State = #state{is_active = false}) ->
    case verify_opts(Opts) of
        {ok, Ms, Apps} ->
			[Starter|PIDs]=read_pids([]),
			NewState = State#state{avg_wait = Ms,
								   ...
								   pids=PIDs,
								   starter=Starter},
			%util:show_message(io_lib:format("Getting ~p processes to kill on node ~p ",[length(PIDs), node()]), Starter),
			util:show_message(io_lib:format("Getting ~p processes to kill on node ~p ",[length(PIDs), node()])),
			self() ! my_kill_something,
			{noreply, NewState};
        {error, _Error} ->
            {noreply, State}
...
handle_info(kill_something, State = #state{avg_wait = AvgWait, apps = Apps}) ->
    case do_kill(Apps) of
        {ok, KilledInfo} ->
            ...
            %%amir
			util:show_message(io_lib:format("Killed process ~p",[KilledInfo]), State#state.starter);
        {error, no_killable_processes} ->
            p("Warning: no killable processes.", []),
            %%amir
			util:show_message("Warning: no killable processes.", State#state.starter)
    end,
...
handle_info(my_kill_something, State = #state{avg_wait = AvgWait, apps = _Apps, pids= PIDs}) ->
	case length(PIDs) of
		0 ->
			util:show_message(io_lib:format("There is no PID to kill on node ~p",[node()]), State#state.starter),
			{noreply, State};
		_ ->
			SortedPIDs=randomize(PIDs),
			[PID|_Tail]=SortedPIDs,
			Reason=kill(PID),
			%%amir
			NewPIDs=lists:delete(PID, PIDs),
			util:show_message(io_lib:format("Chaos Monkey kills a process ~p with reason ~p on node ~p - remain length is ~p",[PID, Reason, node(), length(PIDs)])),
			Var = 0.3, %% I.e. 70% to 130% of Waittime
			WaitTime = round(AvgWait * ((1 - Var) + (Var * 2 * random:uniform()))),
			{ok, Ref} = timer:send_after(WaitTime, my_kill_something),
			{noreply, State#state{timer_ref = Ref, pids= NewPIDs}}
		%_->
		%	{noreply, State}
	end;

handle_info({pid, NewPID}, State = #state{pids= PIDs}) ->
	NewPIDs=PIDs++[NewPID],
...
is_killable(Pid, App, AppFilter, IsSupervisorKillable)
  when is_pid(Pid), is_atom(App), is_boolean(IsSupervisorKillable) ->
      %%amir
    %util:show_message(io_lib:format("is killable process ~p ~n",[Pid]), State#state.starter),
    (App =:= undefined
     orelse
     case AppFilter of
         all -> true;
